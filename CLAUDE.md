...
- 既存6モード（grass/aquarium/tokaido/takibi/skating/shark）はOSSのまま。
...
| プロンプト要約 | 対応内容 |
|---|---|
| 窓モードをペンディングにし、ソースコードから切り離してペンディングルームに隔離 | `pending/window/` を作成。`scenes/window.py` を移動し、main.py / i18n.py / scenes/__init__.py / platform_win.py / 1f.spec の窓関連差分を `window_mode.patch` として保存後、git checkout で除去。窓と無関係な 1f.spec のシーン hiddenimport 追加のみ再適用。復元手順を README.md に記載。 |
